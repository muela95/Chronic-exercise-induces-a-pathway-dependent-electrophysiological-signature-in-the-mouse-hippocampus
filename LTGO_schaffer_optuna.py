...
def permutation_test(X, y, groups, feature_names, model_params, n_permutations=1000):
    ...
    baseline_accuracy = analyze_schaffer_features_with_params(X, y, groups, feature_names, model_params)
    ...
    for i in range(n_permutations):
        ...
        perm_acc = analyze_schaffer_features_with_params(X, y_perm, groups, feature_names, model_params)
...
def load_schaffer_data(file_path):
    """Load and process Schaffer experimental data from .mat file"""
    logger.info("Loading Schaffer data...")
    ...
    logger.info(f"Schaffer: {len(control)} control, {len(exercised)} exercised animals")
...
def create_schaffer_dataset_with_caching(control, exercised, window_size, overlap):
    ...
    for i, animal_data in enumerate(control):
        animal_id = f'schaffer_control_{i}'
    ...
    for i, animal_data in enumerate(exercised):
        animal_id = f'schaffer_exercised_{i}'
...
def analyze_schaffer_features_with_params(X, y, groups, feature_names, model_params):
...
def objective_with_convergence_monitoring(trial, control, exercised, study):
    ...
    try:
        ...
        X, y, groups, feature_names, animal_features = create_schaffer_dataset_with_caching(
        ...
        accuracy = analyze_schaffer_features_with_params(X, y, groups, feature_names, model_params)
...
def analyze_optimization_results(study):
    ...
    # Save results
    trials_df.to_csv('optuna_schaffer_optimization_results.csv', index=False)
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('optuna_schaffer_optimization_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
...
def final_analysis_with_best_params(control, exercised, study):
    ...
    # Save to files
    with open('optuna_schaffer_best_config.json', 'w') as f:
        json.dump(optimization_summary, f, indent=2, default=str)
    
    # Save VIF analysis
    vif_data.to_csv('multicollinearity_analysis_schaffer.csv', index=False)
    
    logger.info(f"\nOptimization complete!")
    logger.info(f"Files saved:")
    logger.info(f"- optuna_schaffer_optimization_results.csv")
    logger.info(f"- optuna_schaffer_best_config.json")
    logger.info(f"- optuna_schaffer_optimization_analysis.png")
    logger.info(f"- multicollinearity_analysis_schaffer.csv")
...
if __name__ == "__main__":
    # Load data
    #file_path_schaffer = 'D:/Laboratorio/Registros/Experimental-ejercicio/Schff8-todo.mat'
    file_path_schaffer = 'D:/Laboratorio/Registros/Experimental-ejercicio/Schff8-todo-nuevo.mat'
    control, exercised = load_schaffer_data(file_path_schaffer)
