...
from sklearn.model_selection import LeavePGroupsOut, cross_val_score, permutation_test_score
...
def iterative_feature_selection(X, y, groups, feature_names, vif_threshold=5.0, max_features=15):
    ...
    if len(remaining_features) > max_features:
        ...
        # Cross-validated importance estimation using LeavePGroupsOut
...
def verify_group_isolation(cv_splitter, groups, y):
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(cv_splitter.split(np.zeros(len(groups)), y, groups)):
        ...
        # Log fold composition for verification
        train_labels = y[train_idx]
        test_labels = y[test_idx]
        
        logger.info(f"Fold {fold_idx}:")
        logger.info(f"  Training subjects: {sorted(train_subjects)} (classes: {np.bincount(train_labels)})")
        logger.info(f"  Test subjects: {sorted(test_subjects)} (classes: {np.bincount(test_labels)})")
    
    logger.info("Group isolation verified - proper cross-validation structure maintained")
...
def cross_validate_model(X, y, groups, model_params):
    """Evaluate model performance using Leave-Two-Groups-Out cross-validation"""
    cv = LeavePGroupsOut(n_groups=2)
...
def comprehensive_model_assessment(X, y, groups, feature_names, model_params, apply_feature_selection=True):
    ...
    # Set up cross-validation framework using Leave-Two-Groups-Out
    cv = LeavePGroupsOut(n_groups=2)
    ...
    logger.info("Conducting Leave-Two-Groups-Out cross-validation assessment...")
    ...
    # Log summary results
    logger.info("\n" + "="*50)
    logger.info("LEAVE-TWO-GROUPS-OUT CROSS-VALIDATION PERFORMANCE SUMMARY")
    logger.info("="*50)
...
def generate_evaluation_plots(results_summary, importance_df, effect_sizes_df):
    ...
    axes[0, 0].set_title('Leave-Two-Groups-Out Performance Metrics')
    ...
    plt.tight_layout()
    plt.savefig('neural_classifier_evaluation_shaffer.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    config_file = 'optuna_schaffer_best_config.json'
    ...
    # data_file = 'D:/Laboratorio/Registros/Experimental-ejercicio/Schff8-todo.mat'
    data_file = 'D:/Laboratorio/Registros/Experimental-ejercicio/Schff8-todo-nuevo.mat'
    ...
    # Calculate number of Leave-Two-Groups-Out folds
    unique_groups = np.unique(groups)
    n_groups = len(unique_groups)
    n_folds = (n_groups * (n_groups - 1)) // 2  # Combinations of n_groups choose 2
    logger.info(f"  Leave-Two-Groups-Out will create {n_folds} folds")
    ...
    evaluation_results = {
        ...
        'dataset_characteristics': {
            ...
            'cv_method': 'Leave-Two-Groups-Out',
            'n_folds': n_folds
        }
    }
    
    # Save evaluation results
    with open('neural_classifier_evaluation_schaffer.json', 'w') as f:
        json.dump(evaluation_results, f, indent=2, default=str)
    
    importance_df.to_csv('feature_importance_rankings_schaffer.csv', index=False)
    effect_sizes_df.to_csv('effect_size_analysis_schaffer.csv', index=False)
    ...
    logger.info(f"Cross-validation method: Leave-Two-Groups-Out ({n_folds} folds)")
    ...
    logger.info(f"- neural_classifier_evaluation_shaffer.json")
    logger.info(f"- feature_importance_rankings_schaffer.csv")
    logger.info(f"- effect_size_analysis_schaffer.csv")
    logger.info(f"- neural_classifier_evaluation_shaffer.png")
